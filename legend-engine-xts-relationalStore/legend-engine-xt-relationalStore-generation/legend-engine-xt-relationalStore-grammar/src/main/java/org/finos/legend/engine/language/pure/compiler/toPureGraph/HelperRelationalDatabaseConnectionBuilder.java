...
public class HelperRelationalDatabaseConnectionBuilder
{
    ...
    public static void addRelationalQueryGenerationConfigs(DatabaseConnection databaseConnection, Root_meta_external_store_relational_runtime_DatabaseConnection pureConnection, CompileContext context)
    {
        ...
        {
            MutableList<Root_meta_relational_runtime_GenerationFeaturesConfig> pureConfigs = Lists.mutable.ofInitialCapacity(databaseConnection.queryGenerationConfigs.size());
            for (RelationalQueryGenerationConfig config : databaseConnection.queryGenerationConfigs)
            {
                if (config instanceof GenerationFeaturesConfig)
                {
                    ...
                    MutableList<? extends String> knownFeatures = core_relational_relational_runtime_relationalRuntimeExtension.Root_meta_relational_runtime_knownGenerationFeatures__String_MANY_(context.getExecutionSupport()).toList();
                    Root_meta_relational_runtime_GenerationFeaturesConfig pureGenerationFeaturesConfig = new Root_meta_relational_runtime_GenerationFeaturesConfig_Impl("", null, context.pureModel.getClass("meta::relational::runtime::GenerationFeaturesConfig"));
                    if (generationFeaturesConfig.enabled != null)
                    {
                        generationFeaturesConfig.enabled.forEach(f ->
                        {
